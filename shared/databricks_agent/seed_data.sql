-- Field-medical reference data for the Databricks agent.
-- Run on a SQL warehouse (default target: main.field_medical).
-- The app queries products + adverse_events; adjust catalog/schema to taste
-- (UC_CATALOG / UC_SCHEMA env vars) if you don't want to use the main catalog.

CREATE SCHEMA IF NOT EXISTS main.field_medical
  COMMENT 'Field-medical demo: product + adverse-event reference data';

CREATE TABLE IF NOT EXISTS main.field_medical.products (
  product_name       STRING,
  generic_name       STRING,
  drug_class         STRING,
  ...
  pregnancy_guidance STRING
) USING DELTA;

CREATE TABLE IF NOT EXISTS main.field_medical.adverse_events (
  product_name  STRING,
...
INSERT INTO main.field_medical.products VALUES
 ('Cardizafen','cardizafen hydrochloride','calcium-channel blocker (antihypertensive)',
  ...
  'In severe renal impairment (CrCl < 30 mL/min), initiate at 90 mg once daily and titrate slowly with close BP and renal monitoring. No dose adjustment required in mild-to-moderate impairment.',
  'Reduce starting dose in moderate-to-severe hepatic impairment; monitor LFTs.',
  'Category C: use only if potential benefit justifies potential risk to the fetus. Limited human data.'),
 ...
 ('Glucoravir','glucoravir sodium','SGLT2 inhibitor (type 2 diabetes)',
  ...
  'Do not initiate if eGFR < 30 mL/min/1.73m2; discontinue if eGFR falls persistently below 45.',
